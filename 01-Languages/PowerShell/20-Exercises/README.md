...
4. **A real class.** Write a `class Book` with `Title`/`Author`/`IsCheckedOut`, and methods `CheckOut()`/`ReturnBook()` that throw if called in the wrong state (e.g. checking out an already-checked-out book). (Note: name the return method something other than `Return` - it's a reserved keyword and cannot be used as a PowerShell class method name; see 21-Solutions for the real parser error this produces.)
